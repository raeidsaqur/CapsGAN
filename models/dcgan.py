...
class DCGAN_D(nn.Module):
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0, bias=True):
        ...
        main.add_module('initial.conv.{0}-{1}'.format(nc, cndf),
                        nn.Conv2d(nc, cndf, 4, 2, 1, bias=self.bias))
        main.add_module('initial.leaky-relu.{0}'.format(cndf), nn.LeakyReLU(0.2, inplace=True))
        ...
        for t in range(n_extra_layers):
            ...
            main.add_module('extra-layers-{0}.{1}.batchnorm'.format(t, cndf),
                            nn.BatchNorm2d(cndf))
            main.add_module('extra-layers-{0}.{1}.leaky-relu'.format(t, cndf),
        ...
        while csize > 4:
            ...
            main.add_module('pyramid.{0}-{1}.conv'.format(in_feat, out_feat), nn.Conv2d(in_feat, out_feat, 4, 2, 1, bias=self.bias))
            main.add_module('pyramid.{0}.batchnorm'.format(out_feat), nn.BatchNorm2d(out_feat))
            main.add_module('pyramid.{0}.leaky-relu'.format(out_feat), nn.LeakyReLU(0.2, inplace=True))
        ...
        main.add_module('final.{0}-{1}.conv'.format(cndf, 1),
                        nn.Conv2d(cndf, 1, 4, 1, 0, bias=self.bias))
...
class DCGAN_G(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0, bias=True, opt=None):
        ...
        main.add_module('initial.{0}-{1}.convt'.format(nz, cngf),
                        nn.ConvTranspose2d(in_channels=nz, out_channels=cngf, kernel_size=4, stride=1, padding=0, bias=self.bias))
